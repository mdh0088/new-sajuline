  ...
  <div class="auth-container">
    <main class="auth-main">
      <div class="max-w-md mx-auto text-center">
        <div class="text-6xl mb-4 font-bold text-green-500">N</div>
        <h2 class="text-2xl font-bold mb-4">네이버 로그인 중...</h2>
...
const callbackMutation = useHandleSocialCallback({
  onSuccess: (data) => {
    ...
    if ('requires_signup' in data && data.requires_signup) {
      ...
      // (네이버 OAuth 페이지와 callback 페이지를 건너뜀)
  ...
  onError: (err: any) => {
    error.value = err?.message || '네이버 로그인에 실패했습니다.'
