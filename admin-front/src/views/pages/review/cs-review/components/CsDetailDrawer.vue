...
import {ref} from 'vue'
import {getCsReviewInfo, updateCsReview } from "@/views/pages/review/cs-review/csReviewPage"
import {CsReivewClass} from "@/models/review"
...
const reviewInfo = ref<CsReviewInfo>(new CsReivewClass());
...
const confirmClick = async () => {
  if (openType.value == "update") {
    ...
    if (swalResult.isConfirmed) {
      await updateCsReview(reviewInfo, props.doSearch as Function)
    }
  }
}
...
const openDrawer = async () => {
  if (openType.value == "update") {
    ...
    reviewInfo.value.idx = props.chooseRow.idx;
    console.log('chk reviewInfo >>>',reviewInfo.value)

    await getCsReviewInfo(reviewInfo)
  }
}
