...
import {dumyReviewHeader, dumySearchOptions} from "@/views/pages/review/dumy-review/dumyReviewConstants"
import {getDumyReviewList, deleteDumyReview} from "@/views/pages/review/dumy-review/dumyReviewPage"
...
const DumyDetailDrawer = defineAsyncComponent(() => import("@/views/pages/review/dumy-review/components/DumyDetailDrawer.vue"))

const openType = ref("update"); // create or update


const activeTab = ref('')
const tableOptions = ref<TableOptions<DumyReviewInfo>>(new TableOptionsClass({headers: dumyReviewHeader}))
const searchOptions = ref(new SearchOptionsClassList(dumySearchOptions))
...
const doSearch = async (sort:string='asc', sortKey:string='') => {
  ...
  await getDumyReviewList(queryParams, tableOptions)
}
