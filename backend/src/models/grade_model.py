...
class Grade(Base):
    ...
    # 등급 조건 및 혜택
    min_purchase_amount: Mapped[int] = mapped_column(
        Integer, 
        nullable=False,
        comment="최소 구매 금액"
    )
    point_earn_rate: Mapped[Decimal] = mapped_column(
        DECIMAL(5, 2), 
        nullable=False,
        comment="포인트 적립률"
    )
    ...
    # 등급 표시 정보
    grade_image_url: Mapped[Optional[str]] = mapped_column(
        String(500), 
        nullable=True,
        comment="등급 이미지 URL"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "point_earn_rate >= 0 AND point_earn_rate <= 100",
            name='chk_point_earn_rate'
        ),
        CheckConstraint(
            "discount_rate >= 0 AND discount_rate <= 100",
            name='chk_discount_rate'
        ),
        CheckConstraint(
            "min_purchase_amount >= 0",
            name='chk_min_purchase_amount'
        ),
        {'comment': '등급 정의'}
    )
