...
include_once $_SERVER['DOCUMENT_ROOT'].'../common/utilities.php';
include_once $_SERVER['DOCUMENT_ROOT']."./naver_lib.php";
...
$user_data = array("USER_ID" => $_SESSION['naver_user_info']['response']['email']);
if($utilities->chkTableInfoByObj('TBL_USER',$user_data)){
    ...
    $utilities -> addLoginLog($_SESSION['IDX'],'naver');
    $utilities -> update_last_login($_SESSION['IDX'],'TBL_USER');
    die(header("Location:".'./sns_login_ok.php'));
}
else
{
    die(header("Location:".$naverlogin->result_uri));
