...
if($data -> REVIEW_TYPE == 'REAL'){
    if ($cs->update_review_reply($data)){

        $chk_data = array("IDX" => $data->IDX);
        $review_info= $utilities->readOne('TBL_CS_REVIEW',$chk_data);

        $chk_data = array("IDX" => $review_info['USER_IDX']);
    ...
    if ($cs->update_review_dumy_reply($data)){
        echo json_encode(array("message" => "저장 되었습니다.","callback"=>"","isSuc"=>TRUE));
        http_response_code(200);
    }
}
