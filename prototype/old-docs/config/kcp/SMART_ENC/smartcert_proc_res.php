...
/*    echo "========================= 복호화 데이터 ====================="       ."<br>";
    ...
    $key = "mysajuro##";
    $encrypted = base64_encode("pchked" ^ $key);

    echo
        "
        <script>
            var tel1 = '".$ct_cert->mf_get_key_value("phone_no")."';
            var tel2 = '".add_hyphen($ct_cert->mf_get_key_value("phone_no"))."';
            var phone_chk  = '".$encrypted."';
        </script>
    ";
        ...
        window.onload=function()
        {
            try
            {
                if (navigator.maxTouchPoints){
                    window.parent.document.getElementById("phone_chk").value=phone_chk;
                    window.parent.btn_disable('Y',tel1);
                }else{
                    window.opener.document.getElementById("phone_chk").value=phone_chk;
                    window.opener.btn_disable('Y',tel1);
                }
                window.close();
                //parent.auth_data( document.form_auth ); // 부모창으로 값 전달

                /*window.parent.document.getElementById("phone_chk").value=phone_chk;
                window.parent.btn_disable();*/
