...
<script>
    ...
    const getBookMarkCS = async () => {
        ...
        try {
            ...
            let result = await axios.post('/api/user/read_bookmark', data);
            console.log(result);
            let inner = '';
...
<section id="csWrap" class="section fast">
    ...
    <div class="subTitBox">
        <h3>관심 상담사</h3>
        <button type="button" class="icoBtn icoRefresh" onclick="getBookMarkCS();">새로고침</button>
    </div>
    <!--// subTitBox -->
...
<script>
    setMeta({
        title: '관심 상담사',
        description:
            '관심 상담사 : 나의 단골선생님만 따로 모아두기 ! No.1 전화 사주/타로상담 서비스 플랫폼 사주로',
        url: 'https://sajutarot.com/app/user/bookmark',
        keyword: ''
    });
</script>
<?php
