...
// URL 파라미터에서 code, state, error 추출
...
onMounted(() => {
  try {
    if (error) {
      // OAuth 에러 발생
      const errorMessage = error_description || '네이버 로그인 중 오류가 발생했습니다.'
  ...
  if (!window.opener && (code || error)) {
    try {
      ...
      if (code && state) {
        ...
        const result = await authAPI.processSocialCallback('naver', code, state)
        
        // 신규 사용자이고 추가 정보가 필요한 경우
        if (result.is_new_user && result.requires_additional_info) {
          console.log('[DEBUG] 네이버 신규 사용자 - 추가 정보 입력 필요:', result)
        ...
        if (processedResult.success) {
          // 소셜 로그인 성공 (기존 사용자)
          console.log('[DEBUG] 네이버 기존 사용자 로그인 성공:', processedResult)
          await navigateTo('/?message=social_login_success')
        } else {
          throw new Error(processedResult.message || '네이버 로그인 처리 실패')
        }
      }
      
    } catch (error: any) {
      console.error('네이버 리다이렉트 콜백 처리 오류:', error)
      await navigateTo(`/login?error=${encodeURIComponent('네이버 로그인 처리 중 오류가 발생했습니다.')}`)
    }
  }
})
</script>
...
<template>
  <div class="min-h-screen bg-slate-950 text-white flex items-center justify-center">
    <div class="text-center">
      ...
      <h1 class="text-xl font-semibold mb-2">네이버 로그인 처리 중...</h1>
