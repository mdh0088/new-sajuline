...
class SystemEvents:
    ...
    def application_started(cls, **kwargs) -> None:
        """애플리케이션 시작"""
        logger.bind(request_id=get_request_id()).info("Admin Application Started", **kwargs)

    @classmethod
    def application_shutdown(cls, **kwargs) -> None:
        """애플리케이션 종료"""
        logger.bind(request_id=get_request_id()).info("Admin Application Shutdown", **kwargs)