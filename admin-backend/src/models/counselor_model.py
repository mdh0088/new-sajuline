...
class Counselor(Base):
    ...
    # 기본키 및 로그인 정보
    counselor_id: Mapped[str] = mapped_column(String(100), primary_key=True, comment="상담사 ID (로그인 ID = 이메일)")
    counselor_code: Mapped[str] = mapped_column(String(50), unique=True, nullable=False, comment="상담사 고유 코드")
    ...
    nickname: Mapped[str] = mapped_column(String(100), unique=True, nullable=False, comment="활동명")
    phone: Mapped[str] = mapped_column(String(50), unique=True, nullable=False, comment="전화번호")
    profile_image_url: Mapped[Optional[str]] = mapped_column(String(500), nullable=True, comment="프로필 이미지")
    introduction_short: Mapped[Optional[str]] = mapped_column(Text, nullable=True, comment="짧은 소개")
    greeting_message: Mapped[Optional[str]] = mapped_column(Text, nullable=True, comment="인사말")
    career_info: Mapped[Optional[str]] = mapped_column(Text, nullable=True, comment="경력사항")

    # 상담 관련 정보
    counselor_status: Mapped[str] = mapped_column(String(20), nullable=False, default="WAITING", comment="상태: WAITING, CONSULTING, ABSENT")
    ...
    # 평가 및 통계
    rating_avg: Mapped[Optional[Decimal]] = mapped_column(DECIMAL(3, 2), nullable=True, default=Decimal("0.00"), comment="평균 평점")
    rating_count: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, default=0, comment="평점 개수")
    consultation_count: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, default=0, comment="총 상담 횟수")
    consultation_time_total: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, default=0, comment="총 상담 시간(분)")
    ...
    is_show: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, comment="노출여부")
